                ...
                <div class="bg-white p-4 shadow-md rounded-md">
                    ...
                    <form class="form-horizontal" method="put" action="{{ route('updateItem', $itemData->item_id) }}">
                        ...
                        <input type="text" id="name" name="name"
                            ...
                            required value="{{ $itemData->name }}"><br />
                        ...
                        <select name="category_id" id=""
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ $category->id == $itemData->category_id ? 'selected' : '' }}>
                                    {{ $category->title }}
                                </option>
                            @endforeach
                        </select><br />
                        ...
                        <textarea id="description" name="description"
                            ...
                            rows="4" required>{{ $itemData->description }}</textarea><br />

                        <!-- Price -->
                        <label for="price">Price:</label>
                        <input type="number"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            id="price" name="price" value="{{ $itemData->price }}" required>

                        <!-- Owner -->
                        <label for="owner">Owner:</label>
                        <input type="text"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            id="owner" name="owner" value="{{ $itemData->owner }}" required><br />
